...
sudo install -d -m 0775 "${BASE}/nginx/logs" "${BASE}/nginx/cache" "${BASE}/nginx/configs" "${BASE}/nginx/certs" 
echo ">> Setting ownership to ${UID_IN_CONTAINER}:${GID_IN_CONTAINER} ..."
sudo chown -R "${UID_IN_CONTAINER}:${GID_IN_CONTAINER}" "${BASE}/nginx/logs" "${BASE}/nginx/cache" "${BASE}/nginx/configs" "${BASE}/nginx/certs" 
